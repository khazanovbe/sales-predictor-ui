...
import { TUI_MONTHS, TuiAlertService, TuiNotification, TuiPoint } from '@taiga-ui/core';
...
export class AppComponent {
    ...
    tuiIconStarLarge = 'tuiIconStarLarge';
    ...
    constructor(private http: HttpClient, private readonly alerts: TuiAlertService) {
            ...
            mergeMap((ticker: string) => {
                return this.http.post<IForecast[]>('/api/evaluate', {
                    ...
                    catchError((e: HttpErrorResponse) => {
                        this.showLoader = false;
                        return this.alerts
                            .open(`Error code: ${e.status}`, { label: 'Server error!' ,status: TuiNotification.Error},);
                    })
                );
            })
        ).subscribe();
